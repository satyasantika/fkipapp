...
@push('header')
    ...
    @if ($examregistration->id &&  !$examregistration->dilaporkan)
        <form id="delete-form" action="{{ route('registrations.destroy',$examregistration->id) }}" method="POST">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-outline-danger btn-sm float-end" title="Hapus" onclick="return confirm('Yakin akan menghapus data {{ $examregistration->exam_type->nama_ujian }} {{ $student->nama }}?');">
