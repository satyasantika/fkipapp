            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="accordion" id="accordionExample">
                        @forelse (App\Models\ExamRegistration::where('departement_id',auth()->user()->departement_id)->selectRaw("DATE_FORMAT(tanggal_ujian,'%Y-%m') as kode_laporan")->distinct()->get()->sortDesc() as $list)
                        <div class="accordion-item">
                            ...
                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $list->kode_laporan }}" aria-expanded="false" aria-controls="collapse{{ $list->kode_laporan }}">
                                @php
                                    $penguji = App\Models\ExamRegistration::whereKodeLaporan($list->kode_laporan)->where('departement_id',auth()->user()->departement_id);
                                ...
                                @if (App\Models\ExamRegistration::where('departement_id',auth()->user()->departement_id)->whereKodeLaporan($list->kode_laporan)->where('dilaporkan',0)->exists())
                            ...
                            <div class="accordion-body">
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        @forelse (App\Models\ExamRegistration::whereKodeLaporan($list->kode_laporan)
                                            ...
                                            <tr>
                                                <td>
                                                    ...
                                                    @if (App\Models\ExamRegistration::where([
                                                ...
                                                <td>
                                                    {{ \App\Models\ExamRegistration::where('tanggal_ujian',$list2->tanggal_ujian)->where('departement_id',auth()->user()->departement_id)->count() }}
                                                    <a href="{{ route('reports.by.date',$list2->tanggal_ujian) }}" class="btn btn-sm btn-outline-primary">detail ujian</a>
                                                </td>
                                                <td>
                                                    @php
                                                        $penguji = App\Models\ExamRegistration::where('tanggal_ujian',$list2->tanggal_ujian)->where('departement_id',auth()->user()->departement_id);
                                                        $pembimbing1 = $penguji->whereNotNull('pembimbing1_id')->pluck('pembimbing1_id');
                                                        $pembimbing2 = $penguji->whereNotNull('pembimbing2_id')->pluck('pembimbing2_id');
                                                        $penguji1 = $penguji->whereNotNull('penguji1_id')->pluck('penguji1_id');
                                                        $penguji2 = $penguji->whereNotNull('penguji2_id')->pluck('penguji2_id');
